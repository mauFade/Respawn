  ...
  async create(request: Request, response: Response) {
    try {
      ...
      if (sameGamertag) {
        ...
        return response.status(403).send({
          message: "Error at method create.",
          error: "This gamertag is already in use.",
        });
      }
    ...
    try {
      ...
      if (!userEmail) {
        ...
        return response.status(500).send({
          message: "Error at method update.",
          error: "Email of user witch data is being updated is necessary.",
        });
      }
      ...
      return response
        .status(200)
        .send({ success: true, data: "User updated successfully!" });
      // Caso algo dê errado
    } catch (error) {
      // Retorna erro
      return response
  ...
  async delete(request: Request, response: Response) {
    try {
      ...
      if (!userEmail) {
        // Retorna erro
        return response.status(403).send({
          message: "Error at method delete.",
      ...
      return response
        .status(200)
        .send({ success: true, data: "User deleted successfully!" });
